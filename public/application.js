...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gameboardRef = __webpack_require__(8);
	var winners = __webpack_require__(9);
	var auth = __webpack_require__(11);
	var api = __webpack_require__(13);
	var store = __webpack_require__(10);

	var playerOne = []; // empty array for player one's turns
	var playerTwo = []; // empty array for player two's turns
	var playerTurn = 0; // player toggle switch
	var gameIndex = ["", "", "", "", "", "", "", "", ""]; // all player moves are stored here
	var gameMove = [];
	var gameState = {};
	var currentGameBoard = gameboardRef.gameboard; // sets a blank game board array
	var gameOver = false;
	var value = "x";
	...
	var logicReset = function logicReset() {
	...
	var boardSet = function boardSet(event) {
	...
	var positionToValue = function positionToValue(currentBox) {
	  var data = currentBox.id;
	  for (var i = 0; i < gameboardRef.gameboardPosition.length; i++) {
	...
	var valueToPosition = function valueToPosition(currentBox) {
	  var data = currentBox.id;
	  for (var i = 0; i < gameboardRef.gameboardPosition.length; i++) {
	    if (data === gameboardRef.gameboardPosition[i].positionName) {
	...
	var xMove = function xMove(currentMove, currentMoveForPatch, currentBox) {
	  playerOne.push({
	    'move': currentMove
	  });
	...
	var oMove = function oMove(currentMove, currentMoveForPatch, currentBox) {
	  playerTwo.push({
	    'move': currentMove
	  });
	  gameIndex.push(currentMoveForPatch);
	...
	var endResults = function endResults(playerOne, playerTwo) {
	  if (winners.checkVictory(playerOne)) {
	    ...
	    $('.box').on('click', playerMove).off();
	    gameOver = true;
	  } else if (gameIndex.length === 9 && !(winners.checkVictory(playerOne) && !winners.checkVictory(playerTwo))) {
	...
	var playerMove = function playerMove(event) {
	  event.preventDefault();
	  var currentBox = this;
	  var currentMove = positionToValue(currentBox);
	  var currentMoveForPatch = valueToPosition(currentBox);
	  for (var i = 0; i < currentGameBoard.length; i++) {
	    if (currentGameBoard[i].position[0] === currentMove[0] && currentGameBoard[i].position[1] === currentMove[1]) {
...
/***/ (function(module, exports) {
	...
	var gameboard = [{ position: [0, 0],
	  state: false }, { position: [0, 1],
	  state: false }, { position: [0, 2],
	  state: false }, { position: [1, 0],
	  state: false }, { position: [1, 1],
	  state: false }, { position: [1, 2],
	  state: false }, { position: [2, 0],
	  state: false }, { position: [2, 1],
	  state: false }, { position: [2, 2],
	  state: false }];
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var isWinZeroZero = function isWinZeroZero(moves) {
	  if (moves.move[0] === 0 && moves.move[1] === 0) {
	    return true;
	  }
	};

	var isWinZeroOne = function isWinZeroOne(moves) {
	  if (moves.move[0] === 0 && moves.move[1] === 1) {
	    return true;
	  }
	};

	var isWinZeroTwo = function isWinZeroTwo(moves) {
	...
	var isWinOneZero = function isWinOneZero(moves) {
	  if (moves.move[0] === 1 && moves.move[1] === 0) {
	    return true;
	  }
	};

	var isWinOneOne = function isWinOneOne(moves) {
	  if (moves.move[0] === 1 && moves.move[1] === 1) {
	    return true;
	  }
	};

	var isWinOneTwo = function isWinOneTwo(moves) {
	...
	var isWinTwoZero = function isWinTwoZero(moves) {
	  if (moves.move[0] === 2 && moves.move[1] === 0) {
	    return true;
	  }
	};

	var isWinTwoOne = function isWinTwoOne(moves) {
	  if (moves.move[0] === 2 && moves.move[1] === 1) {
	    return true;
	  }
	};

	var isWinTwoTwo = function isWinTwoTwo(moves) {
	...
	var isWinThreeZero = function isWinThreeZero(moves) {
	  if (moves.move[0] === 0 && moves.move[1] === 0) {
	    return true;
	  }
	};

	var isWinThreeOne = function isWinThreeOne(moves) {
	  if (moves.move[0] === 1 && moves.move[1] === 0) {
	    return true;
	  }
	};

	var isWinThreeTwo = function isWinThreeTwo(moves) {
	  if (moves.move[0] === 2 && moves.move[1] === 0) {
	    return true;
	  }
	};
	...
	var isWinFourZero = function isWinFourZero(moves) {
	  if (moves.move[0] === 0 && moves.move[1] === 1) {
	    return true;
	  }
	};

	var isWinFourOne = function isWinFourOne(moves) {
	  if (moves.move[0] === 1 && moves.move[1] === 1) {
	    return true;
	  }
	};

	var isWinFourTwo = function isWinFourTwo(moves) {
	  if (moves.move[0] === 2 && moves.move[1] === 1) {
	    return true;
	  }
	};
	...
	var isWinFiveZero = function isWinFiveZero(moves) {
	  if (moves.move[0] === 0 && moves.move[1] === 2) {
	    return true;
	  }
	};

	var isWinFiveOne = function isWinFiveOne(moves) {
	  if (moves.move[0] === 1 && moves.move[1] === 2) {
	    return true;
	  }
	};

	var isWinFiveTwo = function isWinFiveTwo(moves) {
	  if (moves.move[0] === 2 && moves.move[1] === 2) {
	    return true;
	  }
	};
	...
	var isWinSixZero = function isWinSixZero(moves) {
	  if (moves.move[0] === 2 && moves.move[1] === 0) {
	    return true;
	  }
	};

	var isWinSixOne = function isWinSixOne(moves) {
	  if (moves.move[0] === 1 && moves.move[1] === 1) {
	    return true;
	  }
	};

	var isWinSixTwo = function isWinSixTwo(moves) {
	  if (moves.move[0] === 0 && moves.move[1] === 2) {
	    return true;
	  }
	};
	...
	var isWinSevenZero = function isWinSevenZero(moves) {
	  if (moves.move[0] === 0 && moves.move[1] === 0) {
	    return true;
	  }
	};

	var isWinSevenOne = function isWinSevenOne(moves) {
	  if (moves.move[0] === 1 && moves.move[1] === 1) {
	    return true;
	  }
	};

	var isWinSevenTwo = function isWinSevenTwo(moves) {
	  if (moves.move[0] === 2 && moves.move[1] === 2) {
	    return true;
	  }
	};
	...
	// Victory check zero
	var checkVictoryZero = function checkVictoryZero(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	...
	// Check Victory One
	var checkVictoryOne = function checkVictoryOne(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	  var checkThree = false;
	...
	// Check Victory Two
	var checkVictoryTwo = function checkVictoryTwo(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	  var checkThree = false;
	...
	// check victory three
	var checkVictoryThree = function checkVictoryThree(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	  var checkThree = false;
	...
	// check victory four
	var checkVictoryFour = function checkVictoryFour(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	  var checkThree = false;
	...
	// check victory five
	var checkVictoryFive = function checkVictoryFive(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	  var checkThree = false;
	...
	// check victory six
	var checkVictorySix = function checkVictorySix(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	  var checkThree = false;
	...
	// check victory seven
	var checkVictorySeven = function checkVictorySeven(playerarr) {
	  var checkOne = false;
	  var checkTwo = false;
	  var checkThree = false;
	...
	var checkVictory = function checkVictory(playerarr) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  if (data.credentials.password === data.credentials.password_confirmation) {
	    api.signUp(data).then(ui.signUpSuccess).catch(ui.signUpFailure(data));
	  } else {
	...
	var createGame = function createGame() {
	  event.preventDefault;
	  api.createGameState().then(ui.createGameSuccess).catch(ui.createGameFailure);
	  api.getStates().then(ui.getStatesSuccess).catch(ui.getStatesFailure);
	};
	...
	var updateGame = function updateGame(data) {
	  event.preventDefault;
	  api.updateGame(data).then(ui.successfulUpdate).catch(ui.failedUpdate);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var createGameState = function createGameState(data) {
	  return $.ajax({
	    method: 'POST',
	    url: config.apiOrigin + 'games',
	    headers: {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var condenceWin = function condenceWin(games) {
	  if (checkWin0(games)) {
	    winsAccum += 1;
	...
	var winLoss = function winLoss(games) {
	  condenceWin(games);
	  condenceLoss(games);
	};

	var getGameStats = function getGameStats(games) {
	  winsAccum = 0;
	  lossAccum = 0;
	  for (var i = 0; i < games.length; i++) {
	    winLoss(games[i]);
	  }
	  var ties = games.length - (winsAccum + lossAccum);
	...
	var getStatesFailure = function getStatesFailure(data) {
	  message("You're not ready to play.");
	};

	var successfulUpdate = function successfulUpdate(data) {
	  // message("Next Player's turn.")
	};

	var failedUpdate = function failedUpdate(data) {
	...
	module.exports = {
	  ...
	  getStatesFailure: getStatesFailure,
